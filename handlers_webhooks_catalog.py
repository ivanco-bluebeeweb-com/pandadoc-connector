...
async def list_webhooks(ctx, params: ListWebhooksParams) -> ActionResult:
    ...
    return ActionResult.ok(WebhookSubscriptionList(items=[_webhook_entity(w) for w in items]))
...
async def get_webhook(ctx, params: GetWebhookParams) -> ActionResult:
    ...
    return ActionResult.ok(_webhook_entity(resp if isinstance(resp, dict) else {}))
...
async def create_webhook(ctx, params: CreateWebhookParams) -> ActionResult:
    ...
    return ActionResult.ok(_webhook_entity(resp if isinstance(resp, dict) else payload), refresh_panels=["pandadoc_dashboard"])
...
async def update_webhook(ctx, params: UpdateWebhookParams) -> ActionResult:
    ...
    return ActionResult.ok(_webhook_entity(resp if isinstance(resp, dict) else {"uuid": params.webhook_id, **payload}), refresh_panels=["pandadoc_dashboard"])
...
async def delete_webhook(ctx, params: DeleteWebhookParams) -> ActionResult:
    ...
    return ActionResult.ok(DeleteResult(id=params.webhook_id, deleted=True), refresh_panels=["pandadoc_dashboard"])
...
async def list_webhook_event_types(ctx, params: ListWebhookEventsParams) -> ActionResult:
    ...
    return ActionResult.ok(resp if resp is not None else {})
...
async def list_catalog_items(ctx, params: ListCatalogItemsParams) -> ActionResult:
    ...
    items = resp.get("results", []) if isinstance(resp, dict) else (resp if isinstance(resp, list) else [])
    return ActionResult.ok(CatalogItemList(items=[_catalog_entity(c) for c in items]))
...
async def create_catalog_item(ctx, params: CreateCatalogItemParams) -> ActionResult:
    ...
    return ActionResult.ok(_catalog_entity(resp if isinstance(resp, dict) else payload), refresh_panels=["pandadoc_dashboard"])
...
async def get_catalog_item(ctx, params: GetCatalogItemParams) -> ActionResult:
    ...
    return ActionResult.ok(_catalog_entity(resp if isinstance(resp, dict) else {}))
...
async def update_catalog_item(ctx, params: UpdateCatalogItemParams) -> ActionResult:
    ...
    return ActionResult.ok(_catalog_entity(resp if isinstance(resp, dict) else {"id": params.item_id, **fields}), refresh_panels=["pandadoc_dashboard"])
...
async def delete_catalog_item(ctx, params: DeleteCatalogItemParams) -> ActionResult:
    ...
    return ActionResult.ok(DeleteResult(id=params.item_id, deleted=True), refresh_panels=["pandadoc_dashboard"])
...
async def list_api_logs(ctx, params: ListApiLogsParams) -> ActionResult:
    ...
    items = resp.get("results", []) if isinstance(resp, dict) else (resp if isinstance(resp, list) else [])
    return ActionResult.ok(ApiLogList(items=[
        ApiLogEntry(
